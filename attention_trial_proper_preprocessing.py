...
bi_lstm_layer = Bidirectional(LSTM(n_a, dropout=rate_drop_lstm, recurrent_dropout=rate_drop_lstm, return_sequences=True))
lstm_layer = LSTM(n_a, dropout=rate_drop_lstm, recurrent_dropout=rate_drop_lstm, return_sequences=True)
cudnn_lstm_layer = CuDNNLSTM(n_a, return_sequences=True)
bi_cudnn_lstm_layer = Bidirectional(CuDNNLSTM(n_a, return_sequences=True))


comment_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
embedded_sequences= embedding_layer(comment_input)
x = bi_cudnn_lstm_layer(embedded_sequences)
x = Dropout(rate_drop_dense)(x)
context = one_step_attention(x)
context = Flatten()(context)
merged = Dense(num_dense, activation=act)(context)
...
plot_model(model, to_file='trial/sentiment_model_plot_diff_with_attention_proper_processing_bi_cudnn.png', show_shapes=True, show_layer_names=True)


STAMP = 'trial/simple_lstm_glove_vectors_diff_with_attention_proper_processing_bi_cudnn_%.2f_%.2f'%(rate_drop_lstm,rate_drop_dense)
print(STAMP)
